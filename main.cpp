...
class Linked_list {
   private:
    class Node {
       ...
       public:
        ...
        void set_next_node(Node* next_node) { this->next_node = next_node; }
    };
   ...
   public:
    ...
    void insert_sorted(const int value) {
        ...
        if (!head || head->get_data() >= node->get_data()) {
            ...
            while (current->get_next_node() &&
                   current->get_next_node()->get_data() < node->get_data()) {
                current = current->get_next_node();
            }
            node->set_next_node(current->get_next_node());
            current->set_next_node(node);
    ...
    int operator[](const int index) const { return get_element(index); }
    ...
    bool operator!=(const Linked_list& list) const { return !(*this == list); }
